...
<body class="bg-gray-100 p-4">
    <div id="receipt" class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-lg">
...
<script>
    window.onload = function () {
        const element = document.getElementById('receipt'); // Elemen yang akan diubah jadi PDF
        const opt = {
            margin: 5,
            filename: 'Laporan.pdf',
            image: { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2 },
            jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' } // Format A4, landscape
        };

        html2pdf().set(opt).from(element).save().then(() => {
            window.close(); // Tutup tab setelah unduhan selesai
        });
    };
</script>
